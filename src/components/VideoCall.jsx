...
const VideoCall = ({ currentUser, otherUser, onClose, callType = 'video', isIncomingCallProp = false }) => {
  ...
  useEffect(() => {
    const initializeCall = async () => {
      try {
        ...
        // Only get user media if we don't already have a stream
        if (!localStream) {
          try {
        ...
        // Socket event listeners for outgoing calls only
        ...
        socket.on('callRejected', (data) => {
          console.log('VideoCall: Received callRejected event:', data);
          handleCallRejected(data);
        });
        
        socket.on('callEnded', (data) => {
          console.log('VideoCall: Received callEnded event:', data);
          handleCallEnded(data);
        });
        
        socket.on('callSignal', (data) => {
          console.log('VideoCall: Received callSignal event:', data);
  ...
  const initiateCall = async () => {
    try {
      ...
      console.log('VideoCall: callRequest event sent');
      ...
      try {
        ...
        // Store the offer to send when call is accepted
      ...
      <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-semibold">
            {isCallActive ? `${callType === 'video' ? 'Video' : 'Voice'} Call` : 'Call'}
          </h3>
          <button
            onClick={onClose}
